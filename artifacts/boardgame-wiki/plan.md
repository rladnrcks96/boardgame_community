...
### Task 6: 위키 본문 편집 — ✅ 완료 (games 테이블 UPDATE RLS 정책 누락 발견·수정 포함)
...
### Task 11: 프로필 페이지 조회 — ✅ 완료 (헤더에 프로필 링크 추가, Playwright workers 제한으로 전체 스위트 안정화 포함)
...
- [x] 모든 테스트 통과: `bun run test`(0개, 정상), `bun run test:e2e` — 40/42 통과, 2개(신규 이메일 가입/중복 이메일)는 Supabase 이메일 발송 레이트리밋으로 보류 (코드는 완성, learnings.md 참고)
- [x] 빌드 성공: `bun run build`
- [x] 회원가입 → 로그인 → 위키 편집/리뷰/게시글/댓글/좋아요 → 프로필에서 활동·업적 확인까지 전체 플로우가 end-to-end 동작 (수동 브라우저 확인 포함)
